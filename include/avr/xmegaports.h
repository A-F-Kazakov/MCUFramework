...
#define MAKE_VIRTUAL_PORT(portName, className, ID) 							\
	class className{																		\
		...
		public:																				\
			...
			static DataT read() { return portName.OUT; }							\
			static void writeDir(DataT value) { portName.DIR = value; }		\
			static DataT readDir() { return portName.DIR; }						\
			...
			static void setDir(DataT value) { portName.DIR |= value; }		\
			static void clearDir(DataT value) { portName.DIR &= ~value; }	\
			static DataT readPin() { return portName.IN; }						\
			static void toggleDir(DataT value) { portName.DIR ^= value; }	\
			enum{Id = ID};																	\
			enum{width = 8};																\
	};
