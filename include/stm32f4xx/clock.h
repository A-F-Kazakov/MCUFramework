...
//	IO_BITFIELD_WRAPPER(RCC->PLLCFGR, PllQ, uint32_t, 24, 4);
	...
	{
		protected:
			static inline bool enableClockSource(uint32_t turnOnMask, uint32_t waitReadyMask, uint32_t clockStartTimeout)
			{
			...
			static inline bool disableClockSource(uint32_t turnOnMask, uint32_t waitReadyMask, uint32_t clockStartTimeout)
			{
				uint32_t timeoutCounter = 0;
				RCC->CR &= ~turnOnMask;
	...
	class HseClock : BaseClock
	{
		public:
			static uint32_t clockFrequency() { return HSE_VALUE; }

			static bool enable()
			{
				return BaseClock::enableClockSource(RCC_CR_HSEON, RCC_CR_HSERDY, HSE_STARTUP_TIMEOUT);
			}

			static bool disable()
			{
				return BaseClock::disableClockSource(RCC_CR_HSEON, RCC_CR_HSERDY, HSE_STARTUP_TIMEOUT);
			}
	};

	class HsiClock : BaseClock
	{
		public:
			static uint32_t clockFrequency() { return 16'000'000u; }

			static bool enable()
			{
				return BaseClock::enableClockSource(RCC_CR_HSION, RCC_CR_HSIRDY, HSE_STARTUP_TIMEOUT);
			}

			static bool disable()
			{
				return BaseClock::disableClockSource(RCC_CR_HSION, RCC_CR_HSIRDY, HSE_STARTUP_TIMEOUT);
			}
	};
	...
	class SysClock
	{
		...
		public:
			static uint32_t maxFrequency() { return 168'000'000u; }
